% Forecast 26: forecast using a high-order AR(p) model
...
M1 = readtable(['DengueSprint2025_AggregatedData_',UF,'.csv']); 
% reads data related to the selected state up to EW 52 2024

M2 = readtable(['DengueSprint2025_AggregatedData_',UF,'_updated_2025.csv']); 
% reads data related to the selected state, from EW 1 2025 to EW 25 2025

M=[M1;M2];



kcc=M{:,2};  % known dengue cases from EW 1 of 2010 to EW 25 of 2025

ind_f26=701+52+52;  % index of the end of EW 25 of 2025 (upper bound observed data for validation 2)

cc=kcc(1:ind_f26);  % observed cases from EW 1 of 2010 to EW 25 of 2025

cc(cc==0)=0.1;  % replaces null value in cc with a small value. This is because we will apply 
% a log2 function to the data values. 

lwea=2*52-25;   % number of EW to be forecast, i.e., from EW 26 of 2025 up to EW 52 2026
...
PP=2*52;  % twice the period of 52 EWs, i.e., 104 EWs
...
indf_ini=665+52+52+52; % time index of the EW 41 2025 = 821
indf_end=716+52+52+52; % time index of the EW 40 2026 = 872

% Writes a CSV file with the known and forecast data

epiweek=[(202541:202552)';(202601:202640)'];
...
LB95=q2p5; 
LB95(LB95==0)=1;  % avoids a zero-case prediction
UB95=q97p5;
UB95(UB95==0)=1;  % avoids a zero-case prediction
LB90=q5;
LB90(LB90==0)=1;  % avoids a zero-case prediction
UB90=q95;
UB90(UB90==0)=1;  % avoids a zero-case prediction
LB80=q10;
LB80(LB80==0)=1;  % avoids a zero-case prediction
UB80=q90;
UB80(UB80==0)=1;  % avoids a zero-case prediction
LB50=q25;
LB50(LB50==0)=1;  % avoids a zero-case prediction
UB50=q75;
UB50(UB50==0)=1;  % avoids a zero-case prediction

T = table(epiweek,median_cases,LB95,UB95,LB90,UB90,LB80,UB80,LB50,UB50);

writetable(T,['..\planilhas\forecast_ARp_',UF,'.csv'],'Delimiter',',')
...
legend('median forecast','LB50','UB50')  
...
legend('median forecast','LB80','UB80')  
xlabel('Time (EW index)')
ylabel('Number of Cases')
...
legend('median forecast','LB90','UB90')  
xlabel('Time (EW index)')
ylabel('Number of Cases')
...
legend('median forecast','LB95','UB95')  
xlabel('Time (EW index)')
ylabel('Number of Cases')
title(['Forecast 95% -  ',UF])
axis([EW_index(1) EW_index(end) 0 2*max97p5])

print(['..\plots\forecast26_ARp_',UF],'-dpdf')
